...
# model_specs = [(1,1)]
# tau_list = [np.linspace(0, .1, 24)]
#taus_list = np.linspace(0, .1, 24)
num_cores = 7
...
for i, (q,k) in enumerate(model_specs):
    result = {}
    def processInput(t):
        c = reject_prob_parrell(covariates, k, q, 1280, tau=t, ntrials=8, more=False, design='C')
        s4 = reject_prob_parrell(covariates, k, q, 1280, tau=t, ntrials=8, more=False, design='S4')
        with open("simulation_5_runtime.txt", "a") as f:
            print("start mt", file=f)
        mt = reject_prob_parrell(covariates, k, q, 1280, tau=t, ntrials=8, more=False, design='MT')
        mt2 = reject_prob_parrell(covariates, k, q, 1280, tau=t, ntrials=8, more=True, design='MT')
        with open("simulation_5_runtime.txt", "a") as f:
